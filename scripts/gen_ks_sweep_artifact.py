...
if DEG:
    ...
    A('<div class="card"><h2>Degraded / livelocked arms &mdash; partial data, not measurements</h2>'
      ...
      '<p>Most never had a healthy phase &mdash; they did not collapse from a peak, they never '
      'reached one. That is why a self-normalised &ldquo;collapsed vs its own peak&rdquo; test '
      'misses them, and why a fixed <code>insn &lt; N</code> cut also flags healthy arms caught '
      'between bursts. The peak distribution separates them with no cutoff at all.</p>'
