...
A("`1024x128x128` and +803% on `128x1024x512`, so that term can be large.")
A("")
A("**The `Δ vs opt3` column — the C2 measurement itself — is unaffected.** All three sweeps set")
A("`serve_timeout=2047` identically on these shapes, so C2-alone stays a clean single-knob")
A("comparison. Only `Δ vs old` is confounded, and only on these four rows.")
A("")
