...
module axi_L2_interleaver
  import mempool_pkg::*;
#(
  parameter int unsigned NumAXIMasters = 1,
  parameter int unsigned NumL2         = 16,
  parameter int unsigned L2Size        = 16777216
) (
  input logic clk_i,
  input logic rst_ni,
  ...
  for (genvar i = 0; i < NumAXIMasters; i++) begin : addr_scrambler
    always_comb begin
      ...
      // AW Channel
      if ((axi_l2_req_interleaved_o[i].aw.addr >= L2MemoryBaseAddr) &&
          (axi_l2_req_interleaved_o[i].aw.addr < L2MemoryEndAddr)) begin
      ...
      // AR Channel
      if ((axi_l2_req_interleaved_o[i].ar.addr >= L2MemoryBaseAddr) &&
          (axi_l2_req_interleaved_o[i].ar.addr < L2MemoryEndAddr)) begin
