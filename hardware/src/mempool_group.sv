...
`endif
  ...
  if (EnableGroupBarrier) begin : gen_group_barrier
    ...
    // Bank encoding 3 was unused -- writes with bank 0/2/3 all fell into the WR_MASK else. Claim it
    // for the group MSHR CSR file, reusing bar_struct as the CSR index: 16 CSRs x 32 bits on an
    // ALREADY-DECODED group-level port, so no new address space and no new crossbar decode. Struct N
    // with bank 1 (barrier target) and struct N with bank 3 (CSR N) never collide because the op
    // distinguishes them. See docs/mshr_runtime_csr_design.md.
