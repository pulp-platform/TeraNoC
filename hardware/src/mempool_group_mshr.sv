...
#(
  ...
  always_comb begin
    ...
    if (MshrNum < ReqPortsTotal) begin
      ...
      for (tile_i = 0; tile_i < NumTilesPerGroup; tile_i++) begin
        for (port_i = 1; port_i < NumRemoteReqPortsPerTile; port_i++) begin
          mshr_start = (tile_i * (NumRemoteReqPortsPerTile - 1) + port_i) * MshrsPerReq;
          mshr_end = (tile_i * (NumRemoteReqPortsPerTile - 1) + port_i + 1) * MshrsPerReq;
  ...
  always_comb begin
    ...
    for (int tile_i = 0; tile_i < NumTilesPerGroup; tile_i++) begin
      for (int port_i = 1; port_i < NumRemoteReqPortsPerTile; port_i++) begin
        if (group_mshr_req_valid_i[tile_i][port_i]) begin
          if (req_is_load[tile_i][port_i] && req_hit_mshr[tile_i][port_i]) begin
            ...
            if (group_mshr_req_ready_o[tile_i][port_i]) begin
              for (int mshr_i = 0; mshr_i < MshrNum; mshr_i++) begin
                if (req_hit_mshr_map[tile_i][port_i][mshr_i]) begin
                  if (mshr_d[mshr_i].sub_reqs_num < MshrMergeReqs) begin
                    mshr_d[mshr_i].sub_reqs[mshr_d[mshr_i].sub_reqs_num].valid = 1'b1;
                    mshr_d[mshr_i].sub_reqs[mshr_d[mshr_i].sub_reqs_num].tile_id = tile_i;
                    mshr_d[mshr_i].sub_reqs[mshr_d[mshr_i].sub_reqs_num].port_id = port_i;
                    mshr_d[mshr_i].sub_reqs[mshr_d[mshr_i].sub_reqs_num].core_id =
                        group_mshr_req_i[tile_i][port_i].wdata.core_id;
                    mshr_d[mshr_i].sub_reqs[mshr_d[mshr_i].sub_reqs_num].meta_id =
                        group_mshr_req_i[tile_i][port_i].wdata.meta_id;
                    mshr_d[mshr_i].sub_reqs[mshr_d[mshr_i].sub_reqs_num].amo =
    ...
    for (int tile_i = 0; tile_i < NumTilesPerGroup; tile_i++) begin
      for (int port_i = 1; port_i < NumRemoteRespPortsPerTile; port_i++) begin
        ...
        if (mshr_noc_resp_valid_i[tile_i][port_i]) begin
          for (int mshr_i = 0; mshr_i < MshrNum; mshr_i++) begin
            if (mshr_d_valid[mshr_i] &&
                mshr_d[mshr_i].state == MSHR_WAIT_RESP &&
                mshr_d[mshr_i].owner_tile == tile_i &&
                mshr_d[mshr_i].owner_port == port_i &&
                mshr_d[mshr_i].owner_core == mshr_noc_resp_i[tile_i][port_i].rdata.core_id &&
                mshr_d[mshr_i].owner_meta == mshr_noc_resp_i[tile_i][port_i].rdata.meta_id) begin
        ...
        if (resp_is_mshr[tile_i][port_i]) begin
          mshr_noc_resp_ready_o[tile_i][port_i] = ~mshr_d[resp_mshr_id[tile_i][port_i]].resp_valid;
        end else begin
