...
/// bank stripe. The producer guarantees containment; the parent checks it in simulation.
...
#(
  ...
  // Request classification depends only on opcode and length, not address arithmetic.
  ...
  generate
    for (t = 0; t < NumTilesPerGroup; t++) begin : gen_req_decode_t
      for (p = 1; p < NumRemoteReqPortsPerTile; p++) begin : gen_req_decode_p
        ...
        // Legal load bursts retain their requested length. Containment is a protocol invariant.
        assign req_len_ge2[t][p] = |req_i[t][p].burst_len[BurstLenWidth-1:1];
        assign len_o[t][p] = (!req_valid_i[t][p] || !is_load_o[t][p])
                            ? BurstLenWidth'(1) : len_raw_o[t][p];
        ...
        assign is_full_burst_o[t][p] = is_load_o[t][p] &&
        ...
        // non-single arm is just which burst class is enabled.
