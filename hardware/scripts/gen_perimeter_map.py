...
def periph_channel(placement, periph_at):
    """The L2 channel that shares the perimeter point with the periph router.

    mempool_system.sv special-cases exactly one AXI master: that channel's chimney
    hangs off the 4-port periph router together with `periphs` and `host`, so it is
    the gateway to the bootrom, the control registers and the host. Which channel
    that is falls out of the placement -- 5 at 4x4, 13 at 8x8 -- so the RTL cannot
    hardcode it, and emit_yml and the package must agree on one value or peripheral
    traffic is routed to a router that is not carrying the peripherals.
    """
    ch = next((g for g, v in placement.items() if v == periph_at), None)
    if ch is None:
        sys.exit(f"error: no channel placed at the periph router point {periph_at}")
    return ch


def render(placement, num_x, num_y, periph_at):
    ...
    L = ["// Copyright 2026 ETH Zurich and University of Bologna.",
         ...
         "  // at its first fetch.",
         f"  localparam int unsigned PeriphHbmChannel = {periph_channel(placement, periph_at)};",
         "",
...
def emit_yml(placement, num_x, num_y, periph_at, l2_base, l2_chan_size, route_algo):
    ...
    """
    n = len(placement)
    periph_ch = periph_channel(placement, periph_at)
...
def main():
    ...
    if args.check:
        for ch in sorted(placement):
            pos, d = placement[ch]
            # A channel serves `share` groups, not one. Printing group==ch is only
            # right at share == 1 (<= 4x4); above that it mislabels the group AND
            # measures the distance from the wrong coordinate.
            gs = served[ch]
            gtxt = " ".join(f"g{g}{group_coord(g, ny)}" for g in gs)
            dtxt = ",".join(str(manhattan(group_coord(g, ny), pos)) for g in gs)
            print(f"    ch{ch:2d} -> {DIR_NAME[d]:5s}{pos}  serves {gtxt}  d={dtxt}")
        return
    ...
    if args.outdir:
        ...
        f.write_text(render(placement, nx, ny, periph_at))
        print(f"wrote {f}")
    else:
        print(render(placement, nx, ny, periph_at))
