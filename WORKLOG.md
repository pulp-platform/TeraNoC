...
## 2026-08-31 -- B x KS grid handed to the GVSoC side as a spec

**Artifact:** *Decode B×KS Run Grid* -- https://claude.ai/code/artifact/b36dddfc-ec06-41c0-b6d8-a076f39d7365
All 104 arms with derived parameters, current state, the build recipe and the knob set. Generated by
`/tmp/claude-620771/gen_plan_artifact.py` from `ks_plan.json`, which is exported from the campaign
grid -- so it regenerates rather than being hand-maintained.

**User decision:** ask the GVSoC session to run the same grid and publish a comparable artifact.

**What was sent, beyond the link:**

* **The derivation rules, not the arm list.** `sharers = B/KS`, `vl = I·sharers·elem/cores` (cap
  512 B), `B >= KS`, `KS <= 8`. If their generator reproduces the triangular grid and the vl cap
  from those rules, the two grids are the same object; comparing arm-for-arm is only meaningful if
  both sides derived them identically.
* **Sequencing.** 4x4 first -- 52 arms, **72 MB** of total weight data, 12 in the band. 8x8 after --
  52 arms, **288 MB**, 4x the cost on a mesh neither side has characterised. If only a subset is
  affordable: the 12 band arms plus their `vl=64` neighbours at the same KS (the neighbours are what
  make it a boundary rather than a point), and columns B=16/B=32, the only two with three KS values
  at matched B.
* **Four comparison-breakers**, all learned the hard way here: `MATMUL_REPEAT` is already divided out
  so cycles are per pass; liveness is retired instructions, never simulator speed (a wedge simulates
  *faster*) or mtimes; the **~100-window detection horizon**; and efficiency must be `ideal/actual`
  with ideal from the data size, never a lane-occupancy counter (which inverted a real ranking for
  us once).
* **Artifact contract:** same arm naming, same efficiency definition, per-arm state distinguishing
  measured / running / did-not-complete, and did-not-complete never averaged in. Same columns means
  the two tables diff directly.
* **One thing they must run blind:** the two low-D band probes. Our numbers being visible is fine
  for the KS ladder, but those probes are only evidence if their result was a prediction. My
  prediction stands as sent -- both degrade.
