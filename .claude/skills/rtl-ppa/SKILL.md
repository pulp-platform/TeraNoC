...
The theme: **the tool builds what you wrote, not what you meant.** Most wins come from expressing
existing parallelism, not from changing behaviour.

### 2.1 Variable array index → one-hot select (N levels → log2 N)

The single biggest win available in arbitrated designs.

```systemverilog
// BEFORE: index is a signal, so the tool cannot know which bank each lane writes.
// It builds a 32-deep priority mux per bank per field (later writes override earlier).
...
// carries the bank match -- so at most ONE lane can ever write a bank. OR has no ordering,
// so the tool builds a balanced tree.
assign sel[b]  = win_oh[b] & lane_accept;
assign rec_v[b] = |sel[b];
rec_addr[b] |= {AddrW{sel[b][s]}} & lane_addr[s];              // ~5 levels
```

**The chain was never resolving a real conflict** — it came from the variable index alone. Before
doing this, prove the one-hot: find the LSB-isolate (`x & (~x + 1)`) in the arbiter and confirm the
candidate carries the bank/target match.

### 2.2 Compute from the REGISTER plus a narrow "fire" term

When a later pass depends only on *whether* an earlier pass fired — not on the value it wrote —
compute it from `_q` plus the fire signal so the two evaluate in **parallel** instead of chaining:

```systemverilog
// BEFORE: pass B reads the mshr_d that pass A wrote -> serial
// AFTER:
assign alloc_fire[e] = grant_q_v[BankOf(e)] && (grant_q_way[BankOf(e)] == WayOf(e));
assign post_state[e] = fire[e] ? NEW_STATE : mshr_q[e].state;
```
...
**This is also the most dangerous transformation in the file — see §4.**

### 2.3 Defer a set and apply it once

If an in-cycle set feeds every downstream guard, record it and OR it in at the end:

```systemverilog
alloc_set[e] = 1'b1;                      // during the pass
...
d_valid = d_valid | alloc_set;            // once, at the end
```

Took the allocation arbiter off the clear chain: **130 of a 174-cell critical path**. Legal only
because no clear can fire on an entry allocated this cycle — *prove the exclusion, state it in the
comment*.

### 2.4 One saturated sum, not chained increments
...
### 2.5 Clock-gate enables dominate — keep them shallow

A gating check is ~0.175 ns tighter than a D-pin check, so inferred clock-gate enables are usually
the worst endpoints. Anything that reaches `*_en` deserves scrutiny first. Corollary: **the enable
must follow the write it gates.** If you defer a write by a cycle and leave its enable keyed on the
old cycle, the write is silently dropped (§4, defect 1).

### 2.6 Don't build what synthesis never reads

A signal whose only consumers sit inside `` `ifndef TARGET_SYNTHESIS `` (debug fields, assertions)
is still fully built in the synthesised netlist. Wrap the *computation*, not just the use.
One such signal was `MshrNum × BurstLenWidth` of logic feeding nothing.

### 2.7 Free wins worth grepping for

Tautological assertions (`(x != 0) == (x != 0)`), signals only ever assigned `'0` then added to
something, functions defined and never called, knobs with one reachable arm, duplicated localparams.
Each is small; together they shrink the file that the hard work has to happen in.
