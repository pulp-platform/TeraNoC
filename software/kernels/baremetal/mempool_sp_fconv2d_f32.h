...
/*
  ...
  b) Calculate its contributions to the SP_FCONV2D_FILTER = 7 output rows using
  one column of the filter c) Slide down the input row by 1, injecting the next
  input scalar element in the tail d) Repeat from b), taking the next colum of
  the filter, until the last column is fetched d2) Repeat from a) for all the
  ...
  from a) Every time a new input row is loaded, a new output row is created. The
  first 6 rows and the last 6 rows do not follow this pattern, thus we wrote
...
#define SP_FCONV2D_MIN(a, b) ((a) < (b) ? (a) : (b))
#define SP_FCONV2D_ROWS R
#define SP_FCONV2D_COLS C
#define SP_FCONV2D_CH CH
#define SP_FCONV2D_FILTER F
#define SP_FCONV2D_N SP_FCONV2D_COLS

void conv3d_CHx7x7(float *o, const float *i, const float *f,
                   const unsigned int num_rows) {

  unsigned int n = 0;
  while (n < SP_FCONV2D_N) {
...
void conv3d_CHx7x7_block(float *o, const float *i, unsigned int num_rows,
  ...
  // Helper variables
  const int lwo = SP_FCONV2D_COLS << 2;
  const int lwi = SP_FCONV2D_COLS + SP_FCONV2D_FILTER - 1;
  const int lwi_pad = (SP_FCONV2D_COLS + SP_FCONV2D_FILTER - 1) << 2;

  // Number of elements that separates two adjacent channels
  const int ich_len = (SP_FCONV2D_ROWS + SP_FCONV2D_FILTER - 1) *
                      (SP_FCONV2D_COLS + SP_FCONV2D_FILTER - 1);
  const int fch_len = SP_FCONV2D_FILTER * SP_FCONV2D_FILTER;
  ...
  for (int ch = 0; ch < SP_FCONV2D_CH; ++ch) {

    // Point to the first element of the channel ch
    i__ = i_ + ich_len * ch;

    // Point to the scalar elements to insert during a slide
    i_slide_ptr_0 = i__ + n_ + 0 * (SP_FCONV2D_COLS + SP_FCONV2D_FILTER - 1);
    i_slide_ptr_1 = i__ + n_ + 1 * (SP_FCONV2D_COLS + SP_FCONV2D_FILTER - 1);
    i_slide_ptr_2 = i__ + n_ + 2 * (SP_FCONV2D_COLS + SP_FCONV2D_FILTER - 1);
    i_slide_ptr_3 = i__ + n_ + 3 * (SP_FCONV2D_COLS + SP_FCONV2D_FILTER - 1);
    ...
    for (int k = 0; k < SP_FCONV2D_FILTER / 2; ++k) {
  ...
  // Bump the input ptr
  i_ += 4 * (SP_FCONV2D_COLS + SP_FCONV2D_FILTER - 1);
  ...
  // Bump the input ptr
  i_ += 3 * (SP_FCONV2D_COLS + SP_FCONV2D_FILTER - 1);
  ...
  // The input matrix has SP_FCONV2D_ROWS + SP_FCONV2D_FILTER - 1 rows
  // We have computed SP_FCONV2D_FILTER input rows already
  // Nompute now until only SP_FCONV2D_FILTER input rows are left
  // (The last SP_FCONV2D_FILTER-1 rows do not contribute to SP_FCONV2D_FILTER
  // output rows each, so keep them outside of this loop) (We keep
  // SP_FCONV2D_FILTER rows outside because of the unrolling by 2, just for
  // easeness) limit: ((SP_FCONV2D_ROWS + SP_FCONV2D_FILTER - 1) - 2 *
  // SP_FCONV2D_FILTER) / 2
  for (unsigned int j = 0; j < (num_rows - SP_FCONV2D_FILTER - 1) / 2; ++j) {
    // Work on SP_FCONV2D_FILTER output rows
    ...
    for (int ch = 0; ch < SP_FCONV2D_CH; ++ch) {
      ...
      for (int k = 0; k < SP_FCONV2D_FILTER / 2; ++k) {
        ...
        // Calculate SP_FCONV2D_FILTER contributions of the input rows, on
        // SP_FCONV2D_FILTER different output rows
        ...
        // Nalculate SP_FCONV2D_FILTER contributions of the input rows, on
        // SP_FCONV2D_FILTER different output rows
    ...
    // Bump the input ptr
    i_ += SP_FCONV2D_COLS + SP_FCONV2D_FILTER - 1;
...
#undef SP_FCONV2D_ROWS
#undef SP_FCONV2D_COLS
#undef SP_FCONV2D_CH
#undef SP_FCONV2D_FILTER
#undef SP_FCONV2D_N
