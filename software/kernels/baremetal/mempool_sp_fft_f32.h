...
inline void fft_p1(float *src, float *buf, const float *twi,
                   const uint32_t nfft, const uint32_t ntwi, const uint32_t cid,
                   const uint32_t num_cores, const uint32_t stage,
                   const uint32_t len) __attribute__((always_inline));
inline void fft_p2(float *s, float *buf, const float *twi, float *out,
                   ...
                   const uint32_t stride, const uint32_t stride_e,
                   const uint32_t ntwi) __attribute__((always_inline));

/* Phase 1 of vector FFT on muti-core */
/* This function would be run several times in main with barrier*/

void fft_p1(float *src, float *buf, const float *twi, const uint32_t nfft,
            const uint32_t ntwi, const uint32_t cid, const uint32_t num_cores,
...
void fft_p2(float *s, float *buf, const float *twi, float *out,
  ...
  for (uint32_t bf = 0; bf < log2_nfft; ++bf) {
    ...
    for (; avl > 0; avl -= vl) {
      ...
      // 2 load/store with 2 calc sometimes gives a better performance (window
      // of 4 insn)
      asm volatile("vle32.v v0, (%0);" ::"r"(re_u_i)); // v0: Re upper wing
      re_u_i += vl;
      asm volatile("vle32.v v4, (%0);" ::"r"(re_l_i)); // v4: Re lower wing
      ...
      asm volatile(
          "vfadd.vv v20, v8, v12"); // v20: Im butterfly output upper wing
      asm volatile(
          "vfsub.vv v4, v8, v12"); // v4: Im butterfly output upper wing
