...
#define ACTIVE_GROUP_DIV 1
  ...
  // NO "M % active_groups" CHECK HERE. That is a PREFILL precondition -- it exists because the
  // prefill scheme hands each group M/active_groups rows. Decode never splits M (M = batch = 32,
  // far below active_groups), so the check would reject every legal decode shape with -4.
  // The decode preconditions live with the distribution below, where they belong.
