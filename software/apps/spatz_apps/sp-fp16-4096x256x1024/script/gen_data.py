...
def emit_GEMM_layer(name="gemm", **kwargs):
    ...
    # _Float16, not __fp16: the kernel ties A elements to an "f" asm operand for the
    # vfmacc.vf broadcast, and only _Float16 is a native arithmetic type on RISC-V zfh.
    # Identical 16-bit storage layout, so nothing about the emitted data changes.
...
def main():
    ...
    if param["kernel"] == "Conv2d":
        ...
        # torch CPU has no fp16 GEMM ("addmm_impl_cpu_ not implemented for Half"), and an
        # fp16-accumulated golden would not be the right reference anyway: we want the
        # exactly-rounded product so the device error is measurable. Accumulate in fp32 and
        # round the result back to the storage precision.
        result = (param["alpha"] * mat_C.float()
                  + torch.matmul(mat_A.float(), mat_B.float())).to(mat_A.dtype)
