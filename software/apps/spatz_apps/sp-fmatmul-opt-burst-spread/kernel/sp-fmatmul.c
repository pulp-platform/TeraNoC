...
//         Zexin Fu, ETH Zurich (burst-spread optimization)
//
// Burst-optimized matrix multiply with tile-spread B layout.
//
// Same as sp-fmatmul-opt-burst-merge but B is stored with a padded row
// stride (B_stride > P) so that consecutive B rows land on different TCDM
// tiles. This spreads burst load traffic across all 16 tiles instead of
// concentrating on 2 tile pairs.
//
// The inner loop uses B_stride for B row stepping. C output still uses P.
...
void matmul_8xVL(float *c, const float *a, const float *b,
  ...
  while (p < p_end) {
    ...
    for (unsigned int m = m_start; m < m_end; m += 8) {
      ...
      const float *b__ = b_ + B_stride;  // padded stride
      ...
      // Final accumulate + store (C uses stride P, not B_stride)
      asm volatile("vfmacc.vf v0, %0, v20" ::"f"(t0));
...
void matmul(float *c, const float *a, const float *b, const unsigned int M,
            const unsigned int N, const unsigned int P) {
  // Not used in the parallel version (main.c calls matmul_8xVL directly)
  if (M <= 4) {
    matmul_2xVL(c, a, b, 0, M, N, P, P, 0, P);
  } else if (M <= 8) {
    matmul_4xVL(c, a, b, 0, M, N, P, P, 0, P);
  } else {
    matmul_8xVL(c, a, b, 0, M, N, P, P, 0, P);
  }
}
