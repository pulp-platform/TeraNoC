...
// correctness signal -- and a kernel that computes garbage twice as fast still wins
// a sweep. This probe costs one printf per group and touches no FP register.
...
static elem_t a_mesh[((NUM_GROUPS) * (A_GROUP_STRIDE)) / (GEMM_ELEM_BYTES)]
...
#define KERNEL_SIZE 8
  ...
  const elem_t *const a_use = a_mesh + (gid / (uint32_t)(A_GROUPS_PER_REPLICA))
