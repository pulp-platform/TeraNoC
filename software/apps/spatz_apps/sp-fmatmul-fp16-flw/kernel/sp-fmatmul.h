...
// DIAGNOSTIC (sp-fmatmul-fp16-flw only): the A-walk scalar type.
//   float    -> the 8 A loads become flw  (4-byte, aligned)
//   _Float16 -> they are flh              (2-byte, sub-word)
// vfmacc.vf at SEW=16 consumes only rs1[15:0] and Spatz applies NO NaN-box check
// (spatz_vfu.sv:312), so a 32-bit load still delivers a usable fp16 operand.
// This build exists to answer ONE question: is the SUB-WORD scalar load the deadlock trigger?
// The computed VALUES are wrong here (A is walked at 2x stride); that is irrelevant to a hang
// test and the warm-up pass already produces garbage C by design.
typedef float ascal_t;
...
void matmul(elem_t *c, const elem_t *a, const elem_t *b, const unsigned int M,
            const unsigned int N, const unsigned int P);

inline void matmul_single_unrolled(elem_t *c, const elem_t *a, const elem_t *b,
                                   const unsigned int N, const unsigned int P,
                                   unsigned int vl)
