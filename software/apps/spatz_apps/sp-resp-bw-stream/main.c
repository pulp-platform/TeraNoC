...
//
// sp-resp-bw-stream: SUSTAINED resp-bound burst-load microbenchmark.
//
// Every active core streams back-to-back, IDENTICAL, REMOTE 16-word vle32 bursts
// (VL=16 / LMUL=1 = one coalescable 64B line) with NO compute and NO per-iteration
// barrier. Because all active cores execute the exact same loads in lockstep they
// coalesce in the group MSHR (one entry, multicast drain) -- the matmul shared-B
// shape -- but with nothing to hide the response latency, so the per-line drain
// (1 beat/entry/cycle) is the EXPOSED, SUSTAINED bottleneck. A 4-way v-reg
// round-robin keeps the VLSU ROB full (double-buffered) so there is no inter-burst
// idle gap. Goal: measure the realized per-core burst-load response bandwidth and,
// via the always-on [BP]/[MSHR stats] + a logged group-0 MSHR, whether the upstream
// (remote banks + NoC) sustains >1 beat/cyc per line (=> a 2-wide drain would pay).
//
// clang-format off
...
#ifndef ACTIVE_CORES
#define ACTIVE_CORES 16          // 0 => all cores; 16 = one group, remote to REMOTE_G
#endif
#ifndef STREAM_ITERS
#define STREAM_ITERS 512u        // back-to-back bursts per core (must be a multiple of 4)
#endif
#ifndef REMOTE_G
...
int main(void) {
  ...
  // Stream geometry: base line in REMOTE_G, step by GROUP_STRIDE so every line stays
  // in REMOTE_G (always remote to group 0). Same sequence for all cores -> coalesces.
  const uint32_t base_w   = line_in_group(REMOTE_G);
  const uint32_t stride_w = GROUP_STRIDE_BYTES / 4u;
